...
let expInfo = {'participant': '<Study ID>'};
...
async function experimentInit() {
  ...
  lang = "EN";
  ContinueArrow_0 = new visual.Polygon ({
  ...
  ContinueArrow_1 = new visual.Polygon ({
  ...
  ContinueArrow_2 = new visual.Polygon ({
...
function IDcodesRoutineBegin(snapshot) {
  return async function () {
    ...
    IDcodesComponents.forEach( function(thisComponent) {
...
function LocationCodesRoutineBegin(snapshot) {
  return async function () {
    ...
    LocationCodesComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function CalibrationRoutineEachFrame() {
  return async function () {
    ...
    if ((soundRED.status === PsychoJS.Status.FINISHED)) {
        soundRED.status = NOT_STARTED;
    }
    if ((soundBLUE.status === PsychoJS.Status.FINISHED)) {
        soundBLUE.status = NOT_STARTED;
    }
...
function UStestRoutineBegin(snapshot) {
  return async function () {
    ...
    UStestComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function USRatingRoutineBegin(snapshot) {
  return async function () {
    ...
    USRatingComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
